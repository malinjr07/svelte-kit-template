...
```perl
//draws all elements with the "draw-me" class appliedgsap.from('.draw-me', { duration: 1, drawSVG: 0 });
```
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/6UAoyBcn2fk" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen=""></iframe>
...
|  #### .getLength( element:\[Element | Selector text\] ) : Number  | Provides an easy way to get the length of an SVG element's stroke including: `<path>`, `<rect>`, `<circle>`, `<ellipse>`, `<line>`, `<polyline>`, and `<polygon>` |
|-------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| #### .getPosition( element:\[Element | Selector text\] ) : Number |                                          Provides an easy way to get the current position of the DrawSVG.                                           |
