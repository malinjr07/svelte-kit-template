...
```php
var tl = gsap.timeline();var tween = gsap.to(element, { duration: 1, x: 100, opacity: 0.5 });tl.add(tween);//this line produces the same result as the previous two lines (just shorter)tl.to(element, { duration: 1, x: 100, opacity: 0.5 });
```
