Before a [`+page.svelte`](https://svelte.dev/docs/kit/routing#page-page.svelte) component (and its containing [`+layout.svelte`](https://svelte.dev/docs/kit/routing#layout-layout.svelte) components) can be rendered, we often need to get some data. This is done by defining `load` functions.

## Page data[](https://svelte.dev/docs/kit/load#Page-data)
...
A `load` function in a `+page.js` file runs both on the server and in the browser (unless combined with `export const ssr = false`, in which case it will [only run in the browser](https://svelte.dev/docs/kit/page-options#ssr)). If your `load` function should _always_ run on the server (because it uses private environment variables, for example, or accesses a database) then it would go in a `+page.server.js` instead.

A more realistic version of your blog post’s `load` function, that only runs on the server and pulls data from a database, might look like this:

src/routes/blog/\[slug\]/+page.server

Notice that the type changed from `PageLoad` to `PageServerLoad`, because server `load` functions can access additional arguments. To understand when to use `+page.js` and when to use `+page.server.js`, see [Universal vs server](https://svelte.dev/docs/kit/load#Universal-vs-server).

## Layout data[](https://svelte.dev/docs/kit/load#Layout-data)
...
## Universal vs server[](https://svelte.dev/docs/kit/load#Universal-vs-server)
...
### When does which load function run?[](https://svelte.dev/docs/kit/load#Universal-vs-server-When-does-which-load-function-run)

Server `load` functions _always_ run on the server.

By default, universal `load` functions run on the server during SSR when the user first visits your page. They will then run again during hydration, reusing any responses from [fetch requests](https://svelte.dev/docs/kit/load#Making-fetch-requests). All subsequent invocations of universal `load` functions happen in the browser. You can customize the behavior through [page options](https://svelte.dev/docs/kit/page-options). If you disable [server-side rendering](https://svelte.dev/docs/kit/page-options#ssr), you’ll get an SPA and universal `load` functions _always_ run on the client.
...
### Input[](https://svelte.dev/docs/kit/load#Universal-vs-server-Input)
...
### Output[](https://svelte.dev/docs/kit/load#Universal-vs-server-Output)

A universal `load` function can return an object containing any values, including things like custom classes and component constructors.

A server `load` function must return data that can be serialized with [devalue](https://github.com/rich-harris/devalue) — anything that can be represented as JSON plus things like `BigInt`, `Date`, `Map`, `Set` and `RegExp`, or repeated/cyclical references — so that it can be transported over the network. Your data can include [promises](https://svelte.dev/docs/kit/load#Streaming-with-promises), in which case it will be streamed to browsers. If you need to serialize/deserialize custom types, use [transport hooks](https://svelte.dev/docs/kit/hooks#Universal-hooks-transport).

### When to use which[](https://svelte.dev/docs/kit/load#Universal-vs-server-When-to-use-which)
...
## Using URL data[](https://svelte.dev/docs/kit/load#Using-URL-data)
...
### url[](https://svelte.dev/docs/kit/load#Using-URL-data-url)
...
> In some environments this is derived from request headers during server-side rendering. If you’re using [adapter-node](https://svelte.dev/docs/kit/adapter-node), for example, you may need to configure the adapter in order for the URL to be correct.

### route[](https://svelte.dev/docs/kit/load#Using-URL-data-route)

Contains the name of the current route directory, relative to `src/routes`:

src/routes/a/\[b\]/\[...c\]/+page

### params[](https://svelte.dev/docs/kit/load#Using-URL-data-params)
...
## Making fetch requests[](https://svelte.dev/docs/kit/load#Making-fetch-requests)
...
## Cookies[](https://svelte.dev/docs/kit/load#Cookies)

A server `load` function can get and set [`cookies`](https://svelte.dev/docs/kit/@sveltejs-kit#Cookies).
...
## Using parent data[](https://svelte.dev/docs/kit/load#Using-parent-data)
...
## Errors[](https://svelte.dev/docs/kit/load#Errors)

If an error is thrown during `load`, the nearest [`+error.svelte`](https://svelte.dev/docs/kit/routing#error) will be rendered. For [_expected_](https://svelte.dev/docs/kit/errors#Expected-errors) errors, use the `error` helper from `@sveltejs/kit` to specify the HTTP status code and an optional message:
...
If an [_unexpected_](https://svelte.dev/docs/kit/errors#Unexpected-errors) error is thrown, SvelteKit will invoke [`handleError`](https://svelte.dev/docs/kit/hooks#Shared-hooks-handleError) and treat it as a 500 Internal Error.
...
## Redirects[](https://svelte.dev/docs/kit/load#Redirects)
...
> [In SvelteKit 1.x](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#redirect-and-error-are-no-longer-thrown-by-you) you had to `throw` the `redirect` yourself

## Streaming with promises[](https://svelte.dev/docs/kit/load#Streaming-with-promises)
...
> [In SvelteKit 1.x](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#Top-level-promises-are-no-longer-awaited) top-level promises were automatically awaited, only nested promises were streamed.

## Parallel loading[](https://svelte.dev/docs/kit/load#Parallel-loading)

When rendering (or navigating to) a page, SvelteKit runs all `load` functions concurrently, avoiding a waterfall of requests. During client-side navigation, the result of calling multiple server `load` functions are grouped into a single response. Once all `load` functions have returned, the page is rendered.

## Rerunning load functions[](https://svelte.dev/docs/kit/load#Rerunning-load-functions)
...
Dependency tracking does not apply _after_ the `load` function has returned — for example, accessing `params.x` inside a nested [promise](https://svelte.dev/docs/kit/load#Streaming-with-promises) will not cause the function to rerun when `params.x` changes. (Don’t worry, you’ll get a warning in development if you accidentally do this.) Instead, access the parameter in the main body of your `load` function.
...
### Untracking dependencies[](https://svelte.dev/docs/kit/load#Rerunning-load-functions-Untracking-dependencies)
...
### Manual invalidation[](https://svelte.dev/docs/kit/load#Rerunning-load-functions-Manual-invalidation)
...
### When do load functions rerun?[](https://svelte.dev/docs/kit/load#Rerunning-load-functions-When-do-load-functions-rerun)
...
-   It declared a dependency on a specific URL via [`fetch`](https://svelte.dev/docs/kit/load#Making-fetch-requests) (universal load only) or [`depends`](https://svelte.dev/docs/kit/@sveltejs-kit#LoadEvent), and that URL was marked invalid with [`invalidate(url)`](https://svelte.dev/docs/kit/$app-navigation#invalidate)
-   All active `load` functions were forcibly rerun with [`invalidateAll()`](https://svelte.dev/docs/kit/$app-navigation#invalidateAll)

`params` and `url` can change in response to a `<a href="..">` link click, a [`<form>` interaction](https://svelte.dev/docs/kit/form-actions#GET-vs-POST), a [`goto`](https://svelte.dev/docs/kit/$app-navigation#goto) invocation, or a [`redirect`](https://svelte.dev/docs/kit/@sveltejs-kit#redirect).
...
## Implications for authentication[](https://svelte.dev/docs/kit/load#Implications-for-authentication)

A couple features of loading data have important implications for auth checks:

-   Layout `load` functions do not run on every request, such as during client side navigation between child routes. [(When do load functions rerun?)](https://svelte.dev/docs/kit/load#Rerunning-load-functions-When-do-load-functions-rerun)
...
-   Use [hooks](https://svelte.dev/docs/kit/hooks) to protect multiple routes before any `load` functions run
...
## Using getRequestEvent[](https://svelte.dev/docs/kit/load#Using-getRequestEvent)
...
Now, you can call `requireLogin` in any `load` function (or [form action](https://svelte.dev/docs/kit/form-actions), for example) to guarantee that the user is logged in:
...
## Further reading[](https://svelte.dev/docs/kit/load#Further-reading)
