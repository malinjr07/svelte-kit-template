...
## CSR[](https://svelte.dev/docs/kit/glossary#CSR)

Client-side rendering (CSR) is the generation of the page contents in the web browser using JavaScript.

In SvelteKit, client-side rendering will be used by default, but you can turn off JavaScript with [the `csr = false` page option](https://svelte.dev/docs/kit/page-options#csr).

## Edge[](https://svelte.dev/docs/kit/glossary#Edge)
...
## Hybrid app[](https://svelte.dev/docs/kit/glossary#Hybrid-app)
...
## Hydration[](https://svelte.dev/docs/kit/glossary#Hydration)
...
In SvelteKit, pages will be hydrated by default, but you can turn off JavaScript with [the `csr = false` page option](https://svelte.dev/docs/kit/page-options#csr).

## ISR[](https://svelte.dev/docs/kit/glossary#ISR)

Incremental static regeneration (ISR) allows you to generate static pages on your site as visitors request those pages without redeploying. This may reduces build times compared to [SSG](https://svelte.dev/docs/kit/glossary#SSG) sites with a large number of pages. You can do [ISR with `adapter-vercel`](https://svelte.dev/docs/kit/adapter-vercel#Incremental-Static-Regeneration).

## MPA[](https://svelte.dev/docs/kit/glossary#MPA)
...
## Prerendering[](https://svelte.dev/docs/kit/glossary#Prerendering)
...
In SvelteKit, you can control prerendering with [the `prerender` page option](https://svelte.dev/docs/kit/page-options#prerender) and [`prerender` config](https://svelte.dev/docs/kit/configuration#prerender) in `svelte.config.js`.

## PWA[](https://svelte.dev/docs/kit/glossary#PWA)

A progressive web app (PWA) is an app that’s built using web APIs and technologies, but functions like a mobile or desktop app. Sites served as [PWAs can be installed](https://web.dev/learn/pwa/installation), allowing you to add a shortcut to the application on your launcher, home screen, or start menu. Many PWAs will utilize [service workers](https://svelte.dev/docs/kit/service-workers) to build offline capabilities.

## Routing[](https://svelte.dev/docs/kit/glossary#Routing)
...
In SvelteKit, client-side routing will be used by default, but you can skip it with [`data-sveltekit-reload`](https://svelte.dev/docs/kit/link-options#data-sveltekit-reload).

## SPA[](https://svelte.dev/docs/kit/glossary#SPA)

A single-page app (SPA) is an application in which all requests to the server load a single HTML file which then does client-side rendering based on the requested URL. All navigation is handled on the client-side in a process called client-side routing with per-page contents being updated and common layout elements remaining largely unchanged. Throughout this site, when we refer to a SPA, we use this definition where a SPA simply serves an empty shell on the initial request. It should not be confused with a [hybrid app](https://svelte.dev/docs/kit/glossary#Hybrid-app), which serves HTML on the initial request. It has a large performance impact by forcing two network round trips before rendering can begin. Because SPA mode has large negative performance and SEO impacts, it is recommended only in very limited circumstances such as when being wrapped in a mobile app.

In SvelteKit, you can [build SPAs with `adapter-static`](https://svelte.dev/docs/kit/single-page-apps).

## SSG[](https://svelte.dev/docs/kit/glossary#SSG)
...
In SvelteKit, you can do static site generation by using [`adapter-static`](https://svelte.dev/docs/kit/adapter-static) or by configuring every page to be prerendered using [the `prerender` page option](https://svelte.dev/docs/kit/page-options#prerender) or [`prerender` config](https://svelte.dev/docs/kit/configuration#prerender) in `svelte.config.js`.

## SSR[](https://svelte.dev/docs/kit/glossary#SSR)
...
In SvelteKit, pages are server-side rendered by default. You can disable SSR with [the `ssr` page option](https://svelte.dev/docs/kit/page-options#ssr).