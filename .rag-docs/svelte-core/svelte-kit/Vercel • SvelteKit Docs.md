...
This adapter will be installed by default when you use [`adapter-auto`](https://svelte.dev/docs/kit/adapter-auto), but adding it to your project allows you to specify Vercel-specific options.

## Usage[](https://svelte.dev/docs/kit/adapter-vercel#Usage)
...
## Deployment configuration[](https://svelte.dev/docs/kit/adapter-vercel#Deployment-configuration)

To control how your routes are deployed to Vercel as functions, you can specify deployment configuration, either through the option shown above or with [`export const config`](https://svelte.dev/docs/kit/page-options#config) inside `+server.js`, `+page(.server).js` and `+layout(.server).js` files.
...
## Image Optimization[](https://svelte.dev/docs/kit/adapter-vercel#Image-Optimization)
...
## Incremental Static Regeneration[](https://svelte.dev/docs/kit/adapter-vercel#Incremental-Static-Regeneration)
...
### expiration[](https://svelte.dev/docs/kit/adapter-vercel#Incremental-Static-Regeneration-expiration)
...
### bypassToken[](https://svelte.dev/docs/kit/adapter-vercel#Incremental-Static-Regeneration-bypassToken)
...
### allowQuery[](https://svelte.dev/docs/kit/adapter-vercel#Incremental-Static-Regeneration-allowQuery)
...
> Pages that are [prerendered](https://svelte.dev/docs/kit/page-options#prerender) will ignore ISR configuration.

## Environment variables[](https://svelte.dev/docs/kit/adapter-vercel#Environment-variables)
...
## Notes[](https://svelte.dev/docs/kit/adapter-vercel#Notes)

### Vercel functions[](https://svelte.dev/docs/kit/adapter-vercel#Notes-Vercel-functions)

If you have Vercel functions contained in the `api` directory at the project’s root, any requests for `/api/*` will _not_ be handled by SvelteKit. You should implement these as [API routes](https://svelte.dev/docs/kit/routing#server) in your SvelteKit app instead, unless you need to use a non-JavaScript language in which case you will need to ensure that you don’t have any `/api/*` routes in your SvelteKit app.

### Node version[](https://svelte.dev/docs/kit/adapter-vercel#Notes-Node-version)
...
## Troubleshooting[](https://svelte.dev/docs/kit/adapter-vercel#Troubleshooting)

### Accessing the file system[](https://svelte.dev/docs/kit/adapter-vercel#Troubleshooting-Accessing-the-file-system)

You can’t use `fs` in edge functions.

You _can_ use it in serverless functions, but it won’t work as expected, since files are not copied from your project into your deployment. Instead, use the [`read`](https://svelte.dev/docs/kit/$app-server#read) function from `$app/server` to access your files. It also works inside routes deployed as edge functions by fetching the file from the deployed public assets location.

Alternatively, you can [prerender](https://svelte.dev/docs/kit/page-options#prerender) the routes in question.

### Deployment protection[](https://svelte.dev/docs/kit/adapter-vercel#Troubleshooting-Deployment-protection)

If using [`read`](https://svelte.dev/docs/kit/$app-server#read) in an edge function, SvelteKit will `fetch` the file in question from your deployment. If you are using [Deployment Protection](https://vercel.com/docs/deployment-protection), you must also enable [Protection Bypass for Automation](https://vercel.com/docs/deployment-protection/methods-to-bypass-deployment-protection/protection-bypass-automation) so that the request does not result in a [401 Unauthorized](https://http.dog/401) response.