...
We highly recommend upgrading to the most recent 1.x version before upgrading to 2.0, so that you can take advantage of targeted deprecation warnings. We also recommend [updating to Svelte 4](https://svelte.dev/docs/svelte/v4-migration-guide) first: Later versions of SvelteKit 1.x support it, and SvelteKit 2.0 requires it.

## redirect and error are no longer thrown by you[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#redirect-and-error-are-no-longer-thrown-by-you)
...
If the error or redirect is thrown inside a `try {...}` block (hint: don’t do this!), you can distinguish them from unexpected errors using [`isHttpError`](https://svelte.dev/docs/kit/@sveltejs-kit#isHttpError) and [`isRedirect`](https://svelte.dev/docs/kit/@sveltejs-kit#isRedirect) imported from `@sveltejs/kit`.

## path is required when setting cookies[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#path-is-required-when-setting-cookies)
...
## Top-level promises are no longer awaited[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#Top-level-promises-are-no-longer-awaited)
...
## goto(...) changes[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#goto()-changes)

`goto(...)` no longer accepts external URLs. To navigate to an external URL, use `window.location.href = url`. The `state` object now determines `$page.state` and must adhere to the `App.PageState` interface, if declared. See [shallow routing](https://svelte.dev/docs/kit/shallow-routing) for more details.

## paths are now relative by default[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#paths-are-now-relative-by-default)

In SvelteKit 1, `%sveltekit.assets%` in your `app.html` was replaced with a relative path by default (i.e. `.` or `..` or `../..` etc, depending on the path being rendered) during server-side rendering unless the [`paths.relative`](https://svelte.dev/docs/kit/configuration#paths) config option was explicitly set to `false`. The same was true for `base` and `assets` imported from `$app/paths`, but only if the `paths.relative` option was explicitly set to `true`.

This inconsistency is fixed in version 2. Paths are either always relative or always absolute, depending on the value of [`paths.relative`](https://svelte.dev/docs/kit/configuration#paths). It defaults to `true` as this results in more portable apps: if the `base` is something other than the app expected (as is the case when viewed on the [Internet Archive](https://archive.org/), for example) or unknown at build time (as is the case when deploying to [IPFS](https://ipfs.tech/) and so on), fewer things are likely to break.

## Server fetches are not trackable anymore[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#Server-fetches-are-not-trackable-anymore)
...
## preloadCode arguments must be prefixed with base[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#preloadCode-arguments-must-be-prefixed-with-base)

SvelteKit exposes two functions, [`preloadCode`](https://svelte.dev/docs/kit/$app-navigation#preloadCode) and [`preloadData`](https://svelte.dev/docs/kit/$app-navigation#preloadData), for programmatically loading the code and data associated with a particular path. In version 1, there was a subtle inconsistency — the path passed to `preloadCode` did not need to be prefixed with the `base` path (if set), while the path passed to `preloadData` did.
...
## resolvePath has been removed[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#resolvePath-has-been-removed)
...
## Dynamic environment variables cannot be used during prerendering[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#Dynamic-environment-variables-cannot-be-used-during-prerendering)
...
## form and data have been removed from use:enhance callbacks[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#form-and-data-have-been-removed-from-use:enhance-callbacks)

If you provide a callback to [`use:enhance`](https://svelte.dev/docs/kit/form-actions#Progressive-enhancement-use:enhance), it will be called with an object containing various useful properties.
...
## Forms containing file inputs must use multipart/form-data[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#Forms-containing-file-inputs-must-use-multipart-form-data)
...
## Generated tsconfig.json is more strict[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#Generated-tsconfig.json-is-more-strict)

Previously, the generated `tsconfig.json` was trying its best to still produce a somewhat valid config when your `tsconfig.json` included `paths` or `baseUrl`. In SvelteKit 2, the validation is more strict and will warn when you use either `paths` or `baseUrl` in your `tsconfig.json`. These settings are used to generate path aliases and you should use [the `alias` config](https://svelte.dev/docs/kit/configuration#alias) option in your `svelte.config.js` instead, to also create a corresponding alias for the bundler.

## getRequest no longer throws errors[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#getRequest-no-longer-throws-errors)
...
## vitePreprocess is no longer exported from @sveltejs/kit/vite[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#vitePreprocess-is-no-longer-exported-from-sveltejs-kit-vite)
...
## Updated dependency requirements[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#Updated-dependency-requirements)
...
## SvelteKit 2.12: $app/stores deprecated[](https://svelte.dev/docs/kit/migrating-to-sveltekit-2#SvelteKit-2.12:-$app-stores-deprecated)

SvelteKit 2.12 introduced `$app/state` based on the [Svelte 5 runes API](https://svelte.dev/docs/svelte/what-are-runes). `$app/state` provides everything that `$app/stores` provides but with more flexibility as to where and how you use it. Most importantly, the `page` object is now fine-grained, e.g. updates to `page.state` will not invalidate `page.data` and vice-versa.

As a consequence, `$app/stores` is deprecated and subject to be removed in SvelteKit 3. We recommend [upgrading to Svelte 5](https://svelte.dev/docs/svelte/v5-migration-guide), if you haven’t already, and then migrate away from `$app/stores`. Most of the replacements should be pretty simple: Replace the `$app/stores` import with `$app/state` and remove the `$` prefixes from the usage sites.
