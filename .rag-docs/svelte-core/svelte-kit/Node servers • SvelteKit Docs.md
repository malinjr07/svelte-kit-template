...
## Usage[](https://svelte.dev/docs/kit/adapter-node#Usage)
...
## Deploying[](https://svelte.dev/docs/kit/adapter-node#Deploying)
...
### Compressing responses[](https://svelte.dev/docs/kit/adapter-node#Deploying-Compressing-responses)
...
However, if you’re building a [custom server](https://svelte.dev/docs/kit/adapter-node#Custom-server) and do want to add a compression middleware there, note that we would recommend using [`@polka/compression`](https://www.npmjs.com/package/@polka/compression) since SvelteKit streams responses and the more popular `compression` package does not support streaming and may cause errors when used.

## Environment variables[](https://svelte.dev/docs/kit/adapter-node#Environment-variables)
...
### PORT, HOST and SOCKET\_PATH[](https://svelte.dev/docs/kit/adapter-node#Environment-variables-PORT-HOST-and-SOCKET_PATH)
...
### ORIGIN, PROTOCOL\_HEADER, HOST\_HEADER, and PORT\_HEADER[](https://svelte.dev/docs/kit/adapter-node#Environment-variables-ORIGIN-PROTOCOL_HEADER-HOST_HEADER-and-PORT_HEADER)
...
If `adapter-node` can’t correctly determine the URL of your deployment, you may experience this error when using [form actions](https://svelte.dev/docs/kit/form-actions):

> Cross-site POST form submissions are forbidden

### ADDRESS\_HEADER and XFF\_DEPTH[](https://svelte.dev/docs/kit/adapter-node#Environment-variables-ADDRESS_HEADER-and-XFF_DEPTH)

The [`RequestEvent`](https://svelte.dev/docs/kit/@sveltejs-kit#RequestEvent) object passed to hooks and endpoints includes an `event.getClientAddress()` function that returns the client’s IP address. By default this is the connecting `remoteAddress`. If your server is behind one or more proxies (such as a load balancer), this value will contain the innermost proxy’s IP address rather than the client’s, so we need to specify an `ADDRESS_HEADER` to read the address from:
...
### BODY\_SIZE\_LIMIT[](https://svelte.dev/docs/kit/adapter-node#Environment-variables-BODY_SIZE_LIMIT)

The maximum request body size to accept in bytes including while streaming. The body size can also be specified with a unit suffix for kilobytes (`K`), megabytes (`M`), or gigabytes (`G`). For example, `512K` or `1M`. Defaults to 512kb. You can disable this option with a value of `Infinity` (0 in older versions of the adapter) and implement a custom check in [`handle`](https://svelte.dev/docs/kit/hooks#Server-hooks-handle) if you need something more advanced.

### SHUTDOWN\_TIMEOUT[](https://svelte.dev/docs/kit/adapter-node#Environment-variables-SHUTDOWN_TIMEOUT)

The number of seconds to wait before forcefully closing any remaining connections after receiving a `SIGTERM` or `SIGINT` signal. Defaults to `30`. Internally the adapter calls [`closeAllConnections`](https://nodejs.org/api/http.html#servercloseallconnections). See [Graceful shutdown](https://svelte.dev/docs/kit/adapter-node#Graceful-shutdown) for more details.

### IDLE\_TIMEOUT[](https://svelte.dev/docs/kit/adapter-node#Environment-variables-IDLE_TIMEOUT)

When using systemd socket activation, `IDLE_TIMEOUT` specifies the number of seconds after which the app is automatically put to sleep when receiving no requests. If not set, the app runs continuously. See [Socket activation](https://svelte.dev/docs/kit/adapter-node#Socket-activation) for more details.

## Options[](https://svelte.dev/docs/kit/adapter-node#Options)
...
### out[](https://svelte.dev/docs/kit/adapter-node#Options-out)

The directory to build the server to. It defaults to `build` — i.e. `node build` would start the server locally after it has been created.

### precompress[](https://svelte.dev/docs/kit/adapter-node#Options-precompress)

Enables precompressing using gzip and brotli for assets and prerendered pages. It defaults to `true`.

### envPrefix[](https://svelte.dev/docs/kit/adapter-node#Options-envPrefix)
...
## Graceful shutdown[](https://svelte.dev/docs/kit/adapter-node#Graceful-shutdown)
...
3.  and finally, close any remaining connections that are still active after [`SHUTDOWN_TIMEOUT`](https://svelte.dev/docs/kit/adapter-node#Environment-variables-SHUTDOWN_TIMEOUT) seconds. ([`server.closeAllConnections`](https://nodejs.org/api/http.html#servercloseallconnections))

> If you want to customize this behaviour you can use a [custom server](https://svelte.dev/docs/kit/adapter-node#Custom-server).
...
-   `IDLE` - shutdown was triggered by [`IDLE_TIMEOUT`](https://svelte.dev/docs/kit/adapter-node#Environment-variables-IDLE_TIMEOUT)

## Socket activation[](https://svelte.dev/docs/kit/adapter-node#Socket-activation)
