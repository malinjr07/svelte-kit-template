...
## Installation[](https://svelte.dev/docs/cli/sv-check#Installation)

You will need to have the `svelte-check` package installed in your project:

## Usage[](https://svelte.dev/docs/cli/sv-check#Usage)

## Options[](https://svelte.dev/docs/cli/sv-check#Options)

### \--workspace <path>[](https://svelte.dev/docs/cli/sv-check#Options-workspace-path)

Path to your workspace. All subdirectories except `node_modules` and those listed in `--ignore` are checked.

### \--output <format>[](https://svelte.dev/docs/cli/sv-check#Options-output-format)

How to display errors and warnings. See [machine-readable output](https://svelte.dev/docs/cli/sv-check#Machine-readable-output).
...
### \--watch[](https://svelte.dev/docs/cli/sv-check#Options-watch)

Keeps the process alive and watches for changes.

### \--preserveWatchOutput[](https://svelte.dev/docs/cli/sv-check#Options-preserveWatchOutput)

Prevents the screen from being cleared in watch mode.

### \--tsconfig <path>[](https://svelte.dev/docs/cli/sv-check#Options-tsconfig-path)
...
### \--no-tsconfig[](https://svelte.dev/docs/cli/sv-check#Options-no-tsconfig)
...
### \--ignore <paths>[](https://svelte.dev/docs/cli/sv-check#Options-ignore-paths)
...
### \--fail-on-warnings[](https://svelte.dev/docs/cli/sv-check#Options-fail-on-warnings)

If provided, warnings will cause `sv check` to exit with an error code.

### \--compiler-warnings <warnings>[](https://svelte.dev/docs/cli/sv-check#Options-compiler-warnings-warnings)

A quoted, comma-separated list of `code:behaviour` pairs where `code` is a [compiler warning code](https://svelte.dev/docs/svelte/compiler-warnings) and `behaviour` is either `ignore` or `error`:

```sql
npx sv check --compiler-warnings "css_unused_selector:ignore,a11y_missing_attribute:error"
```

### \--diagnostic-sources <sources>[](https://svelte.dev/docs/cli/sv-check#Options-diagnostic-sources-sources)
...
### \--threshold <level>[](https://svelte.dev/docs/cli/sv-check#Options-threshold-level)
...
## Troubleshooting[](https://svelte.dev/docs/cli/sv-check#Troubleshooting)
...
## Machine-readable output[](https://svelte.dev/docs/cli/sv-check#Machine-readable-output)
...
## Credits[](https://svelte.dev/docs/cli/sv-check#Credits)

-   Vue’s [VTI](https://github.com/vuejs/vetur/tree/master/vti) which laid the foundation for `svelte-check`

## FAQ[](https://svelte.dev/docs/cli/sv-check#FAQ)

### Why is there no option to only check specific files (for example only staged files)?[](https://svelte.dev/docs/cli/sv-check#FAQ-Why-is-there-no-option-to-only-check-specific-files-(for-example-only-staged-files))
