...
The way you write your styles and build layouts can have a major impact on [Core Web Vitals](https://web.dev/explore/learn-core-web-vitals). This is particularly true for [Cumulative Layout Shift (CLS)](https://web.dev/articles/cls) and [Largest Contentful Paint (LCP)](https://web.dev/articles/lcp).
...
![Screenshot of example site](https://web.dev/static/articles/css-web-vitals/image/screenshot-example-site-4021271c85244.png)

## Layout

### Inserting content into the DOM

Inserting content into a page after the surrounding content has already loaded pushes everything else on the page down. This causes [layout shifts](https://web.dev/articles/cls#layout_shifts_in_detail).

[Cookie notices](https://web.dev/articles/cookie-notice-best-practices), particularly those placed at the top of the page, are a common example of this problem. Other page elements that often cause this type of layout shift when they load include ads and embeds.
...
![Lighthouse's 'Avoid large layout shifts' audit](https://web.dev/static/articles/css-web-vitals/image/lighthouses-avoid-large-e23e7b8778696.png)

The cookie notice is not listed in these findings because the cookie notice itself isn't shifting when it loads. Rather, it causes the items below it on the page (that is, `div.hero` and `article`) to shift. For more information on identifying and fixing layout shifts, see [Debugging Layout Shifts](https://web.dev/articles/debug-layout-shifts).

#### Fix

Place the cookie notice at the bottom of the page using absolute or fixed positioning.

![Cookie notice displayed at bottom of page](https://web.dev/static/articles/css-web-vitals/image/cookie-notice-displayed-828eae20003e.png)
...
Another way to fix this layout shift would be to reserve space for the cookie notice at the top of the screen. This approach is equally effective. For more information, see [Cookie notice best practices](https://web.dev/articles/cookie-notice-best-practices).

## Images

### Images and Largest Contentful Paint (LCP)

Images are commonly the Largest Contentful Paint (LCP) element on a page. Other [page elements that can be the LCP element](https://web.dev/articles/lcp#what-elements-are-considered) include text blocks and video poster images. The time at which the LCP element loads determines LCP.
...
![Diagram highlighting the page's LCP element in different scenarios.](https://web.dev/static/articles/css-web-vitals/image/diagram-highlighting-pag-ec3a0c08181ee.png)
...
![Lighthouse's 'Image elements have explicit width and height' audit](https://web.dev/static/articles/css-web-vitals/image/lighthouses-image-eleme-564156a3dada5.png)
...
By default, a browser will not immediately render a text element if its associated web fonts have not loaded yet. This is done to prevent a ["flash of unstyled text" (FOUT)](https://en.wikipedia.org/wiki/Flash_of_unstyled_content). In many situations, this delays [First Contentful Paint (FCP)](https://web.dev/articles/fcp). In some situations, this delays Largest Contentful Paint (LCP).
...
![Diagram showing a layout shift caused by a font swap](https://web.dev/static/articles/css-web-vitals/image/diagram-showing-layout-s-9f7571069ba78.png)
...
![Screenshot of a font displayed in DevTools](https://web.dev/static/articles/css-web-vitals/image/screenshot-a-font-displa-d77133b394f86.png)
...
![Screenshot of 'Timing' tab in DevTools](https://web.dev/static/articles/css-web-vitals/image/screenshot-timing-tab-3d2f97c4599d8.png)
...
![Screenshot of 'Initiator' tab in DevTools](https://web.dev/static/articles/css-web-vitals/image/screenshot-initiator-t-4206d89fa12fb.png)
...
The primary way that animations affect Web Vitals is when they cause layout shifts. There are two types of animations that you should avoid using: [animations that trigger layout](https://web.dev/articles/animations-guide#triggers) and "animation-like" effects that move page elements. Typically these animations can be replaced with more performant equivalents by using CSS properties like [`transform`](https://developer.mozilla.org/docs/Web/CSS/transform), [`opacity`](https://developer.mozilla.org/docs/Web/CSS/opacity), and [`filter`](https://developer.mozilla.org/docs/Web/CSS/filter). For more information, see [How to create high-performance CSS animations](https://web.dev/explore/animations).

### Identify

The Lighthouse "Avoid non-composited animations" audit may be helpful for identifying non-performant animations.

![Lighthouse's 'Avoid non-composited animations' audit](https://web.dev/static/articles/css-web-vitals/image/lighthouses-avoid-non-c-2449748998fa1.png)
...
Stylesheets are render-blocking. This means that the browser encounters a stylesheet, it will stop downloading other resources until the browser has downloaded and parsed the stylesheet. This may delay LCP. To improve performance, consider [removing unused CSS](https://css-tricks.com/how-do-you-remove-unused-css-from-a-site/), [inlining critical CSS](https://web.dev/articles/extract-critical-css), and [deferring non-critical CSS](https://web.dev/articles/defer-non-critical-css#optimize).

## Conclusion

Although there is still room for further improvements (for example, using [image compression](https://web.dev/articles/use-imagemin-to-compress-images) to deliver images more quickly), these changes have significantly improved the Web Vitals of this site. If this were a real site, the next step would be to [collect performance data from real users](https://web.dev/articles/vitals-measurement-getting-started#measuring_web_vitals_using_rum_data) to assess whether it is [meeting the Web Vitals thresholds for most users](https://web.dev/articles/vitals-measurement-getting-started#data_interpretation). For more information about Web Vitals, see [Learn Web Vitals](https://web.dev/explore/learn-core-web-vitals).